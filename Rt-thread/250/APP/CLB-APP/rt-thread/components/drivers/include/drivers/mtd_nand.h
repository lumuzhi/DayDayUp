...
struct rt_mtd_nand_device
{
    ...
    void *priv;
    NAND_HandleTypeDef hnand1,hnand2;
};

struct rt_mtd_nand_driver_ops
{
    rt_err_t (*read_id)(struct rt_mtd_nand_device *device, rt_uint8_t *id);
...
rt_inline rt_uint32_t rt_mtd_nand_read_id(struct rt_mtd_nand_device *device, rt_uint8_t *id)
{
    RT_ASSERT(device->ops->read_id);
    return device->ops->read_id(device, id);
}
